<template>
  <el-card shadow="never" class="card-box">
      ...
      <el-row :gutter="20">
        ...
        <el-col :sm="24" :md="12" :lg="8" :xl="6">
          <el-form-item label="日志类型">
            <el-select v-model="queryForm.logType" placeholder="请选择日志类型" clearable>
              ...
              <el-option label="详情查询" :value="4" />
              <el-option label="所有列表" :value="5" />
              <el-option label="分页列表" :value="6" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :sm="24" :md="12" :lg="8" :xl="6">
        ...
        <el-col :sm="24" :md="12" :lg="8" :xl="6">
          <el-form-item label="创建时间">
            <custom-date-picker type="daterange" v-model:startDate="queryForm.requestTime"
              v-model:endDate="queryForm.responseTime" clearable start-placeholder="开始时间" end-placeholder="结束时间" />
          </el-form-item>
        </el-col>
        <el-col :sm="24" :md="12" :lg="8" :xl="6">
    ...
    <el-table :data="tableData.data" border style="width: 100%" row-key="id">
      <el-table-column prop="traceId" label="日志链路ID" align="center" />
      ...
      <el-table-column prop="logType" label="响应状态" align="center" />
      <el-table-column prop="username" label="操作人" align="center" />
      <el-table-column prop="requestIp" label="请求IP" align="center" />
      <el-table-column prop="ipAreaDesc" label="IP归属地" align="center" />
      ...
      <el-dialog :model-value="true" destroy-on-close :title="dialogData.title" @close="closeDialog">
        <div>
          ...
          <div class="align-center">
            ...
            <div class="content">{{ status.logType[form.logType]?.label }}</div>
          </div>
          <div class="align-center">
            <div class="label">响应信息</div>
            <div v-if="form.responseSuccess" class="content"><span style="color:#67c23a">成功</span> {{ form.responseCode }}
              {{ form.responseMessage }}</div>
            <div v-else class="content"><span style="color:#f56c6c">失败</span> {{ form.responseCode }} {{
              form.responseMessage }}</div>
          </div>
          <div class="align-center">
            <div class="label">用户环境</div>
            <div class="content">{{ form.isMobile ? '移动端' : 'PC端' }} {{ form.platformName }} {{ form.browserName }}</div>
          </div>
          <div class="align-center">
          ...
          <div v-if="form.exceptionMessage" class="align-center">
            ...
            <div class="content"><el-input v-model="form.exceptionMessage" :autosize="{ minRows: 2, maxRows: 15 }"
                type="textarea" disabled /></div>
          </div>
        </div>
        <template #footer>
